var apiURL= "https://map.co-creators.website/api/";
var map;
var geocoder;
var userGeoData;
...
function plotPlaces() {
  ...
  .then (function(result){
    for (i=0; i < result.length; i++) {
      var icon = "https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=" + i18n_data.categorydata.representation_characters[result[i].represents-1] + "|" + config.markerColors[result[i].primaryarea-1] +"|FFFFFF|FF0000"
      var isMarked = false;
      if (isMarkerSelected(result[i].id)) {
        isMarked = true;
        icon = "https://chart.googleapis.com/chart?chst=d_map_xpin_letter&chld=pin_star|" + i18n_data.categorydata.representation_characters[result[i].represents-1] + "|" + config.markerColors[result[i].primaryarea-1] +"|FFFFFF|FF0000";
      }
...
//   }
  content = content + "<div class='row'><input id='markerbutton' class='select-btn select-btn-secondary' style='width: 80%;' type='button' onclick='selectMarker()' value='" + $.i18n('select') + "'></div>";
  content = content + "<hr>"
...
function selectMarker() {
  if (currentMarker[0].ismarked) {
    ...
    currentMarker[0].setIcon("https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=" + i18n_data.categorydata.representation_characters[currentMarker[0].represents-1] + "|" + currentMarker[0].pincolor + "|FFFFFF|FF0000")
  }
  else {
    currentMarker[0].ismarked = true;
    markedArray.push(currentMarker[0].personid)
    $("#markerbutton").val($.i18n('deselect'));
    currentMarker[0].setIcon("https://chart.googleapis.com/chart?chst=d_map_xpin_letter&chld=pin_star|" + i18n_data.categorydata.representation_characters[currentMarker[0].represents-1] + "|" + currentMarker[0].pincolor + "|FFFFFF|FF0000")
  }
...
function setLocation() {
  ...
  callAPI("GET", "extra/geolocate", "", "")
  .then(function(result) {
    userGeoData = JSON.parse(result)
    var initialLocation = new google.maps.LatLng(userGeoData.geoplugin_latitude, userGeoData.geoplugin_longitude);
  ...
  $.ajax({
    ...
    success : function(data) {
      userGeoData = data;
      var initialLocation = new google.maps.LatLng(data.latitude, data.longitude);
      map.setCenter(initialLocation);
      map.setZoom(10);
      zoom = 10;
      //console.log(data)
...
$(document).ready(function(){
  ...
  $("#register").click(function(e) {
    $("#countryId").val(userGeoData.geoplugin_countryName).change();
  ...
  $("#languages").bind( "change", function(e) {
    ...
    $.getJSON(config.languageurl + "/" + lang + ".json", function (data) {
      ...
      $("#caseslink").attr("href", i18n_data.links.caseslink);
    });
  });  

  $("#countryId").bind( "change", function(e) {
    $("#stateId").empty()
    const country = countrydata.filter((e) => e.country == $(this).val());
    $.each(country[0].states, function(index, value){
  ...
  $.getJSON("./static/country_state.json", function (data) {
    ...
    $.each(countrydata, function(index, value){
      $("#countryId").append('<option value="' + value.country + '">' + value.country +'</option>');
   });
  });
...
function getJsonObjectByValue(arr, value) {
